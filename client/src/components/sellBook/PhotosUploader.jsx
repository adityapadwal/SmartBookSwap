    ...
    return (
        <>
            ...
            <TextField
                ...
                margin="normal"
                variant="outlined"
                InputProps={{ // Configure the underlying Input element
            ...
            <ImageList sx={{ width: 700, height: 175 }} cols={4} rowHeight={175}>
                {addedPhotos.length > 0 && addedPhotos.map((link) => (
                    <ImageListItem key={link}>
                        ...
                        />
                        {/* Buttons */}
                        <div style={{ position: 'absolute', bottom: 0, right: 0, display: 'flex', flexDirection: 'row', justifyContent: 'space-between', width: '100%' }}>
                            {/* mark as main photo (thumbnail) */}
                            <button onClick={(event) => { setAsThumbNail(event, link) }} style={{ cursor: 'pointer', color: 'white', backgroundColor: 'rgba(0, 0, 0, 0.5)', width: '24px', height: '24px' }}>
                                {link === addedPhotos[0] && (
                            ...
                            {/* delete photo */}
                            <button onClick={(event) => { deletePhoto(event, link) }} style={{ cursor: 'pointer', color: 'white', backgroundColor: 'rgba(0, 0, 0, 0.5)', width: '24px', height: '24px' }}>
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-6 h-6" >
